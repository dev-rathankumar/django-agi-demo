...
# MANAGER SYSTEM PROMPT --> Manager agent's job description
...
def execute_tool(tool_name, tool_input, conversation_id=None):
    ...
    if tool_name == "escalate_to_manager":
        ...
        print("Case summary==> ", case_summary)
...
def run_support_agent(user_message, conversation_id, order_id, user_id):
    ...
    print("agent running in thread: ", threading.current_thread().name)
        ...
        if response.stop_reason == 'tool_use':
            ...
            for block in response.content:
                if block.type == 'tool_use':
                    ...
                    # Publish tool call
                    event = {"type": "tool_call", "message": f"Calling {block.name} with {block.input}"}
                    publish(conversation_id, event)

                    # Log tool call
                    AgentLog.objects.create(conversation=conv, event_type='tool_call', message=f"Calling {block.name} with {block.input}")

                    # Execute the tool
                    result = execute_tool(block.name, block.input, conversation_id)

                    print(f"{block.name} returned: {str(result)[:200]}")
            ...
            print(f"Final reply: ", final_text[:200])

            # Log final reply
            AgentLog.objects.create(conversation=conv, event_type="final", message=final_text[:200])
            return final_text
...
def run_manager_agent(case_summary, conversation_id):
    ...
    print("Manager: ", "Case received for review")
    ...
    # manager starts with the case summary from maya
    manager_messages = [
        ...
        if response.stop_reason == 'tool_use':
            ...
            for block in response.content:
                if block.type == 'tool_use':
                    ...
                    print("Manager: ", "Consulting risk agent for fraud assessment")
            ...
            # Manager decision
            event = {"type": "manager", "message": f"Decision: {decision[:150]}"}
            publish(conversation_id, event)

            # Log final decision
            AgentLog.objects.create(conversation=conv, event_type="manager", message=f"Decision: {decision[:150]}")
            return decision
...
def run_risk_agent(user_id, conversation_id):
    ...
    event = {"type": "risk", "message": f"Starting fraud risk assessment for user {user_id}"}
    publish(conversation_id, event)

    print(f"Starting fraud risk assessment for user {user_id}")

    # Log assessment started
    AgentLog.objects.create(conversation=conv, event_type="risk", message=f"Starting fraud risk assessment for user {user_id}")
    ...
    while True:
        ...
        if response.stop_reason == 'tool_use':
            ...
            for block in response.content:
                if block.type == 'tool_use':
                    # Publish risk profile event
                    event = {"type": "risk", "message": f"Calling {block.name} to get customer risk profile"}
                    publish(conversation_id, event)

                    print(f"Calling {block.name} to get customer risk profile")
            ...
            # Risk assessment started
            event = {"type": "risk", "message": f"Verdict: {verdict[:150]}"}
            publish(conversation_id, event)
